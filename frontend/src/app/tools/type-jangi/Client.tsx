...
import { Trophy, ChevronRight, HelpCircle, StopCircle } from 'lucide-react';
          ...
          {!startTime && (
            <div className="absolute inset-0 flex items-center justify-center bg-background/60 backdrop-blur-sm pointer-events-none z-40 text-center">
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <AnimatePresence>{isEnglishKeyboard && <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-primary font-bold text-sm bg-primary/10 px-4 py-1 rounded-full border border-primary/20">هشدار: زبان کیبورد شما انگلیسی است!</motion.span>}</AnimatePresence>
              </div>
            </div>
          )}
        </div>
