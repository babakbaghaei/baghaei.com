...
import { RotateCw, Trophy, Plus, X, ChevronRight } from 'lucide-react';
...
export default function SpinWin() {
  ...
  const drawWheel = useCallback(() => {
    ...
    options.forEach((opt, i) => {
      ...
      // overlay برنده: بستن با Escape و قفل اسکرول پس‌زمینه تا اسکرول به‌هم‌نریزد.
      useEffect(() => {
        if (!winner) return;
        ...
        <main className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/30 flex flex-col items-center py-12 px-6 md:py-24 relative overflow-x-hidden">
          ...
          <div className="w-full max-w-4xl relative z-10 flex flex-col items-center space-y-12 md:space-y-16">
            ...
            <Link
              href="/tools"
              dir="rtl"
              className="self-start -mb-12 inline-flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors text-xs font-display"
            >
      ...
      <div className="mt-32 text-center pb-20 opacity-20 hover:opacity-100 transition-opacity cursor-default">
        {/* decorative wordmark — kept <12px: tracking-[0.5em] overflows 320px at text-xs */}
