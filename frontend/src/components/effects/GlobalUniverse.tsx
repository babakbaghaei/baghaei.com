    ...
    >
      ...
      {planet.hasMoon && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none" style={{ width: 30, height: 30 }}>
          <div className="absolute inset-0 border border-solid border-white/5 rounded-full" />
...
const Planet = ({ planet, scrollProgress }: { planet: any, scrollProgress: MotionValue<number> }) => {
  ...
  const rotate = useTransform(scrollProgress, [0, 1], [baseAngle, baseAngle + (rotationDir * (3652.5 * 2 / planet.period) * 360)]);

  return (
    <div className="absolute pointer-events-none" style={{ width: pDist * 2, height: pDist * 2, left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
      <div className="absolute inset-0 border border-solid border-white/5 rounded-full" style={{ borderWidth: '1px', opacity: 0.15 }} />
...
export default function GlobalUniverse() {
  ...
  const smoothProgress = useSpring(scrollYProgress, { stiffness: 20, damping: 30, mass: 2 });
  
  // Cosmic Journey: Translate the whole system vertically as we scroll
  // Start near top-right, move towards bottom-left
  const x = useTransform(smoothProgress, [0, 1], ["30%", "-20%"]);
  const y = useTransform(smoothProgress, [0, 1], ["5%", "85%"]);
  
  // Scale the whole universe slightly for a depth effect
  const scale = useTransform(smoothProgress, [0, 0.5, 1], [1, 1.2, 0.8]);
  const opacity = useTransform(smoothProgress, [0, 0.05, 0.95, 1], [0.4, 1, 1, 0.4]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#020203]">
      ...
      <motion.div 
        ...
        className="absolute w-[1200px] h-[1200px] right-0 top-0 flex items-center justify-center"
      >
        <div className="relative w-full h-full flex items-center justify-center">
          {/* THE SUN - 40px with massive bloom */}
          <div className="absolute bg-yellow-500 rounded-full shadow-[0_0_200px_rgba(234,179,8,0.3)] z-10" style={{ width: 40, height: 40 }}>
