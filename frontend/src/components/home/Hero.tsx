    ...
    <section id="hero" className="relative min-h-screen flex flex-col items-start justify-center pt-32 bg-transparent">
      ...
      >
        <div className="space-y-12 flex flex-col items-start">
          ...
          >
            <div className="w-full flex flex-col items-start text-right gap-4">
              <div className="h-[60px] md:h-[100px] lg:h-[140px] flex items-center justify-start whitespace-nowrap overflow-visible">
                <span className="text-zinc-500 font-bold" key={`l1-${displayTextLine1.length}`}>{displayTextLine1}</span>
                {phase === 'line1' && (
                  <motion.div 
                    animate={{ opacity: [0.4, 0.4, 0, 0] }}
                    transition={{ duration: 1.1, repeat: Infinity, times: [0, 0.5, 0.5, 1] }}
                    className="w-[2px] md:w-[3px] h-[0.8em] bg-white mr-1.5"
                  />
                )}
              </div>

              <div className="h-[60px] md:h-[100px] lg:h-[140px] flex items-center justify-start whitespace-nowrap overflow-visible">
                <span className="text-white font-black" key={`l2-${displayTextLine2}`}>
          ...
          <motion.p variants={itemVariants} initial="hidden" animate="visible" className="max-w-2xl text-lg md:text-xl text-zinc-400 font-sans leading-relaxed text-right">
